...
Loss is calculated in standardised target space. MAE, RMSE and R² are
...
Repeat regression training using increasing percentages of the available
training reactions.

Validation and test sets remain fixed so performance can be compared across
training-set sizes.
"""
...
Plot test R² against the percentage of the available training data used for
training and save the resulting learning-curve figure.
"""
function plot_learning_curve(learning_curve)
    ...
    p = plot(
        ...
        ylabel = "Test R²",
        title = "Glucose CRN Learning Curve",
        label = "Directed HGNN",
        legend = :bottomright,
        grid = true,
        xticks = percentages,
